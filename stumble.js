...
console.clear('');
const asciiText = figlet.textSync("NO LIMIT", {
    font: 'ANSI Shadow',
    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color(Username), color(Crowns, "blueBright"), color(SkillRating, "blueBright")].join(" | ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
